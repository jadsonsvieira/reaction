...
def get_db_connection():
    try:
        ...
        # Se a base de dados não existir, tenta criar
...
def criar_tabelas_se_nao_existirem():
    ...
    if not conn:
        print("Aviso: Falha ao obter conexão para criar tabelas. O MySQL pode estar desligado.")
        return
    ...
    try:
        ...
        # Criar dados Mock seguros para evitar erro de Foreign Key no ID 1
        ...
        cursor.close()
        conn.close()
        print("Tabelas da base de dados inicializadas com sucesso.")
        
    except Exception as e:
        print(f"Erro fatal ao criar tabelas: {e}")

# Executa a inicialização das tabelas no arranque
try:
    criar_tabelas_se_nao_existirem()
except Exception as e:
    print(f"Ignorando criação de tabelas (BD pode não estar pronta): {e}")
...
def login():
    if request.method == 'POST':
        # Higienização: strip() remove espaços acidentais no início/fim
        email = request.form.get('email', '').strip().lower()
        senha = request.form.get('senha', '')
        
        conn = get_db_connection()
        if not conn:
            return render_template('login.html', erro="Erro temporário no servidor. Tente novamente.")
...
def registo():
    ...
    conn = get_db_connection()
    if not conn:
        return render_template('login.html', erro_registo="Erro no servidor. Tente novamente.", mostrar_registo=True)
...
def reputacao():
    ...
    return render_template('reputacao.html', usuario=usuario)
...
def sala_maquinas():
    ...
    return render_template('sala_maquinas.html', usuario=usuario)
...
def integracoes():
    ...
    return render_template('integracoes.html', usuario=usuario)
...
def auth_callback(plataforma):
    ...
    # Guardar a chave no Banco de Dados
    conn = get_db_connection()
    if conn:
        cursor = conn.cursor()
...
def webhook_ingest():
    # ---------------------------------------------------------
    # Validação do Webhook (Facebook/Instagram)
    # ---------------------------------------------------------
    if request.method == 'GET':
    ...
    try:
        ...
        # ---------------------------------------------------------
        # PARSE ADAPTATIVO: Suporta Teste Local E Webhook Real Meta
        # ---------------------------------------------------------
        if 'entry' in data:
            try:
                plataforma = 'instagram'
                empresa_id = 1 # Em produção, derivar do page_id
                
                alteracao = data['entry'][0]['changes'][0]['value']
                nome_cliente = alteracao.get('from', {}).get('username', 'Utilizador Instagram')
                comentario = alteracao.get('text', '')
                nota = 3 # Instagram não tem nota nativa
            except (KeyError, IndexError):
                return jsonify({"error": "Formato de payload Meta inválido"}), 400
        else:
            # Estrutura do script local de teste
        ...
        if nota >= 4:
            ...
            # GATILHO DE CRISE SEVERA: Cria automaticamente uma Ação Crítica
            data_hoje = date.today().strftime('%Y-%m-%d')
            titulo_crise = f"Contenção de Crise: Avaliação de {nota}★ de {nome_cliente}"
            
            cursor.execute("""
                INSERT INTO acoes (empresa_id, criado_por, titulo, prioridade, prazo, status)
                VALUES (%s, NULL, %s, 'critical', %s, 'pendente')
            """, (empresa_id, titulo_crise, data_hoje))

        # 4. Atualização Mestra: Salva a análise da IA
        cursor.execute("""

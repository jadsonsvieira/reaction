...
def login_microsoft():
    ...
    ms_email = request.values.get('email')
    ms_nome = request.values.get('nome') or request.values.get('name')
    ms_sub = request.values.get('sub') or request.values.get('microsoft_id')
    ...
    try:
        ...
        if not user:
            nome_empresa = f"Empresa de {ms_nome}"
            ...
            try:
                cursor.execute(
                    ...
                    (empresa_id, ms_nome, ms_email, senha_aleatoria, ms_sub)
                )
            except Exception:
                cursor.execute(
                    "INSERT INTO usuarios (empresa_id, nome, email, senha, role) VALUES (%s, %s, %s, %s, 'dono')",
                    (empresa_id, ms_nome, ms_email, senha_aleatoria)
                )
            ...
            try:
                enviar_email_boas_vindas(ms_email, ms_nome, nome_empresa)
                enviar_notificacao_admin("novo_usuario", {
                    "nome_usuario": ms_nome,
                    "email": ms_email,
                    "nome_empresa": nome_empresa,
